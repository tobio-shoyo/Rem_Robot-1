...
import TG_ROBOT.modules.fun_strings as fun_strings
...
async def _(td):
    try:
        resp = (random.choice(fun_strings.TRUTH_STRINGS))
        return await td.reply_text(resp)
    except Exception:
...
async def _(dr):
    try:
        resp = random.choice(fun_strings.DARE_STRINGS).json()
        results = f"{resp['message']}"
        return await dr.reply_text(resp)
    except Exception:
